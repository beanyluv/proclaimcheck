...
export default function BerandaPage() {
  ...
  // Real-time server sync via Supabase Realtime Subscription
  useEffect(() => {
    const sync = async () => {
    ...
    if (supabase) {
      channel = supabase
        ...
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'uploads' }, // Listen to all uploads database events
          async () => {
            const docs = await syncDocs();
            setAllDocuments([...docs]);
          }
  ...
  const terverifikasi = filteredDocuments.filter((doc: any) => doc.status === 'Layak').length;
  const pending = filteredDocuments.filter((doc: any) => doc.status === 'Pending' || doc.status === 'Menunggu Review' || !doc.status).length;
  const ditolak = filteredDocuments.filter((doc: any) => doc.status === 'Tidak Layak').length;
  ...
  const fullPuskesmasData = puskesmasList.map(puskesmasName => {
    ...
    const verified = puskesmasDocs.filter((doc: any) => doc.status === 'Layak').length;
    const pendingCount = puskesmasDocs.filter((doc: any) => doc.status === 'Pending' || doc.status === 'Menunggu Review' || !doc.status).length;
    const rejected = puskesmasDocs.filter((doc: any) => doc.status === 'Tidak Layak').length;
  ...
  const monthlyData = months.map(month => {
    ...
    return {
      month,
      verified: monthDocs.filter((doc: any) => doc.status === 'Layak').length,
      pending: monthDocs.filter((doc: any) => doc.status === 'Pending' || doc.status === 'Menunggu Review' || !doc.status).length,
      rejected: monthDocs.filter((doc: any) => doc.status === 'Tidak Layak').length
    };
  });
    ...
    <>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="col-span-2 bg-[#fcfffa] border-2 border-[#1f6f5f] rounded-[17px] p-6 shadow-md">
          ...
          <div className="h-56 flex items-end justify-around gap-2 border-b border-[#ddd] pb-2 mt-12">
            {monthlyData.map((data, idx) => {
              const maxHeight = 120;
              const maxValue = Math.max(...monthlyData.map(d => Math.max(d.verified, d.pending, d.rejected)));
              ...
              return (
                <div key={data.month} className="flex-1 flex flex-col items-center gap-1 group">
                  <div className="w-full flex items-end justify-center gap-1 h-[150px]">
                    {data.verified > 0 ? (
                      <div className="flex flex-col items-center">
                        <span className="text-[10px] font-bold text-[#2fa084] mb-0.5">{data.verified}</span>
                        <div
                          className="w-3 bg-[#2fa084] rounded-t transition-all duration-300 hover:bg-[#25826b] hover:scale-110 cursor-pointer shadow-sm hover:shadow-lg"
                          style={{ height: `${Math.max(data.verified * scale, 8)}px` }}
                    ...
                    {data.pending > 0 ? (
                      <div className="flex flex-col items-center">
                        <span className="text-[10px] font-bold text-[#ebbd14] mb-0.5">{data.pending}</span>
                        <div
                          className="w-3 bg-[#ebbd14] rounded-t transition-all duration-300 hover:bg-[#d4aa0f] hover:scale-110 cursor-pointer shadow-sm hover:shadow-lg"
                          style={{ height: `${Math.max(data.pending * scale, 8)}px` }}
                    ...
                    {data.rejected > 0 ? (
                      <div className="flex flex-col items-center">
                        <span className="text-[10px] font-bold text-[#f94141] mb-0.5">{data.rejected}</span>
                        <div
                          className="w-3 bg-[#f94141] rounded-t transition-all duration-300 hover:bg-[#e03030] hover:scale-110 cursor-pointer shadow-sm hover:shadow-lg"
                          style={{ height: `${Math.max(data.rejected * scale, 8)}px` }}
                  ...
                  <p className="text-[12px] text-[#5e5e5e] opacity-70 group-hover:font-semibold transition-all mt-1">{monthShort}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-[#fcfffa] border-2 border-[#1f6f5f] rounded-[17px] p-6 shadow-md">
              ...
              return (
                <>
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <p className="text-[26px] font-extrabold text-[#1f6f5f] leading-none">{total}</p>
                    <p className="text-[9px] text-[#8b8b8b] uppercase tracking-wider font-bold mt-1.5">Total Berkas</p>
                  </div>
                </>
              );
            })()}
          ...
          {(() => {
            const total = terverifikasi + pending + ditolak;
            const vPct = total > 0 ? Math.round((terverifikasi / total) * 100) : 0;
            const pPct = total > 0 ? Math.round((pending / total) * 100) : 0;
            const rPct = total > 0 ? Math.round((ditolak / total) * 100) : 0;
            return (
              <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#2fa084]"></div>
                    <span className="text-[12px] text-[#5e5e5e] font-['Mukta']">Terverifikasi</span>
                  </div>
                  <span className="text-[12px] font-bold text-[#2fa084]">{vPct}% ({terverifikasi})</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#ebbd14]"></div>
                    <span className="text-[12px] text-[#5e5e5e] font-['Mukta']">Pending</span>
                  </div>
                  <span className="text-[12px] font-bold text-[#ebbd14]">{pPct}% ({pending})</span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full bg-[#f94141]"></div>
                    <span className="text-[12px] text-[#5e5e5e] font-['Mukta']">Ditolak</span>
                  </div>
                  <span className="text-[12px] font-bold text-[#f94141]">{rPct}% ({ditolak})</span>
                </div>
              </div>
            );
          })()}
      ...
      <Dialog open={showPuskesmasModal} onOpenChange={setShowPuskesmasModal}>
        <DialogContent className="w-full max-w-4xl max-h-[85vh] overflow-hidden bg-white rounded-[24px] shadow-2xl">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {fullPuskesmasData.map((item, index) => (
                <div key={index} className="bg-[#f8faf7] rounded-[15px] p-4 border border-[#dbe9e0]">
                  ...
                  <div className="h-2.5 bg-gray-200 rounded-full overflow-hidden flex mb-3.5">
                    <div className="bg-[#2fa084]" style={{ width: `${item.percentage > 0 ? (item.verified / (item.verified + item.pending + item.rejected)) * item.percentage : 0}%` }}></div>
                    <div className="bg-[#ebbd14]" style={{ width: `${item.percentage > 0 ? (item.pending / (item.verified + item.pending + item.rejected)) * item.percentage : 0}%` }}></div>
                    <div className="bg-[#f94141]" style={{ width: `${item.percentage > 0 ? (item.rejected / (item.verified + item.pending + item.rejected)) * item.percentage : 0}%` }}></div>
                  </div>
